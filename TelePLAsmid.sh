...
echo "                        (%                        "
echo "                                                  "
echo "         Binomica Labs - TelePlasmid v. 0.01      "
echo "                                                  "
...
if [ -d "./TPLA_output" ]
then
        echo "TPLA_output directory already exists. Exiting for now."
        exit
else
        mkdir TPLA_output
fi
...
cp "$1"_input_protein_pfam.txt *.pf TPLA_output/
rm *.pf

#Screen preformatted plasmids pfam output for similar protein acccession numbers
...
zcat allplasmid-pf33.1.pfam.gz | grep -i $input_pfam | awk '{print $2}' > "$1"_similar_protein_accessions.acc && echo "Protein accessions list created" || echo "Protein accession list compilation failed" 
cp "$1"_similar_protein_accessions.acc TPLA_output/
...
cat "$1"_similar_protein_accessions.acc | JoinIntoGroupsOf 200 | xargs -n 1 sh -c \
	...
	cp "$1"_similar_proteins.seq TPLA_output/
...
cat "$1"_similar_protein_accessions.acc | JoinIntoGroupsOf 200 | xargs -n 1 sh -c \
	...
	cp "$1"_similar_proteins_cds.seq TPLA_output/
...
epost -input "$1"_similar_protein_accessions.acc -db protein | efetch -format gb -mode xml | xtract -pattern \
	...
	cp "$1"_similar_proteins_annotation.txt TPLA_output/
...
cp "$1"_similar_proteins.aln TPLA_output/
...
cp "$1"_similar_proteins.tre TPLA_output/
...
zgrep -f "$1"_similar_protein_accessions.acc plasmid-prot.accession2taxid.gz | awk '{print $2"\t"$3}' > "$1"_similar_proteins_to_uids && echo "Protein accessions to taxonomic ID list completed" || echo "Protein accessions to taxonomic ID list failed"
cp "$1"_similar_proteins_to_uids TPLA_output/

#Screening input_proteins_to_uids file to isolate uid for taxonomy tagging

cat "$1"_similar_proteins_to_uids | awk '{print $2}' > "$1"_plasmids_with_similar_proteins.uid && echo "Plasmid taxonomic ID list completed" || echo "Plasmid taxonomic ID list compilation failed"
cp "$1"_plasmids_with_similar_proteins.uid TPLA_output/

#Using Taxonkit to link NCBI uid with taxonomic names for identified plasmid
#Also creating a temporary linear uid file to create taxonomic tree list

taxonkit lineage "$1"_plasmids_with_similar_proteins.uid > "$1"_plasmids-taxonomic-full.txt && echo "Plasmids taxonomic name list completed" || echo "Plasmids taxonomic name compilation failed"
taxonkit lineage -t "$1"_plasmids_with_similar_proteins.uid > "$1"_plasmids-taxonomic-lineage.txt && echo "Plasmids taxonomic lineage list completed" || echo "Plasmids taxonomic lineage compilation failed"

cp "$1"_plasmids-taxonomic-full.txt "$1"_plasmids-taxonomic-lineage.txt TPLA_output/


#Clean up for the working directory

rm "$1"_input_protein_pfam.txt "$1"_similar_protein_accessions.acc "$1"_similar_proteins.seq "$1"_similar_proteins_cds.seq "$1"_similar_proteins_annotation.txt "$1"_similar_proteins_to_uids "$1"_plasmids_with_similar_proteins.uid "$1"_plasmids-taxonomic-full.txt "$1"_plasmids-taxonomic-lineage.txt "$1"_similar_proteins.aln "$1"_similar_proteins.tre && echo "All TelePlasmid processes completed. Have a nice day!" || echo "TelePlasmid process terminated with error" 
